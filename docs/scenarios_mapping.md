...
五、相对方信用超额风险分析（集中授信爆雷风险）(已支持✅)
实现逻辑：
- [x] 1. 构建多维度关联图
  - 同一法人代表：查询 LEGAL_PERSON 边，聚合同一人担任法人的公司
  - 同一联系电话：查询 Person.phone 字段，聚合相同电话关联的公司
  - 高管交叉任职：查询 EMPLOYED_BY 边，识别在多家公司任职的人员
  - 控股关系链：查询 CONTROLS 边，识别母子公司关系
- [x] 2. 聚合马甲集群
  - 使用 Union-Find 算法合并有重叠的公司集合
  - 过滤：集群规模 >= 2
  - 可选排除：央企内部组织关系（ORG_xxx）
- [x] 3. 计算集群特征
  - 授信敞口：集群内所有公司的合同总金额
  - 内部交易：集群内公司之间的交易金额
  - 外部风险：行政处罚、经营异常事件统计
...
- [x] 4. 风险评分模型
  - 集群规模分（25%）：公司数量 >= 5 得满分
  - 授信集中度（30%）：敞口金额 / 阈值（默认1000万）
  - 隐蔽性指标（25%）：1 - (控制人数 / 公司数)，越低越隐蔽
  - 内部交易比（20%）：内部交易 / 总敞口
  - 附加分：有行政处罚或经营异常 +5%
- [x] 5. 生成报告
  - shell_entity_cluster_report.csv：集群维度报告
  - shell_entity_controller_report.csv：控制人维度报告
