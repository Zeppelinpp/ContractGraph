...
基于 Nebula Graph 图数据库构建央企穿透式监督知识图谱，支持七类高级分析场景：
...
6. **外部风险事件分析**：整合 DaaS 外部数据（行政处罚、经营异常）进行风险分析【新增】
7. **人员任职网络分析**：分析人员在多家公司的任职情况，识别关联方网络【新增】
...
| status | 状态 | C:已审核 |
| email | 邮箱 | emp0001@cnbuild-mail.com |
| phone | 电话 | 13962000001 |
...
#### AdminPenalty（行政处罚节点）【新增】
| 属性 | 说明 | 示例 |
|------|------|------|
| node_id | 节点ID | PEN_0001 |
| event_type | 事件类型 | AdminPenalty |
| event_no | 处罚文号 | 中国证券监督管理委员会广东监管局行政处罚决定书〔2023〕20号 |
| event_name | 处罚名称 | 行政处罚 |
| amount | 处罚金额 | 48000.0 |
| status | 状态 | C:已完成 |
| register_date | 处罚日期 | 2023-09-27 |
| description | 详细描述 | 包含违法类型、违法事实、处罚内容、处罚依据等 |

#### BusinessAbnormal（经营异常节点）【新增】
| 属性 | 说明 | 示例 |
|------|------|------|
| node_id | 节点ID | ABN_0001 |
| event_type | 事件类型 | BusinessAbnormal |
| event_no | 记录编号 | 56498534 |
| event_name | 记录名称 | 3643a510ab02472ef9da14b77c8b55a1 |
| amount | 金额 | 0.0 |
| status | 状态 | C:已移出 |
| register_date | 列入日期 | 2017-07-07 |
| description | 详细描述 | 包含列入原因、列入机关、移出原因、移出机关、移出日期等 |
...
| TRADES_WITH | Company → Company | 交易关系 | 200 |
...
| HAS_PARTY | Contract → Company | 合同参与方反向边（自动生成） | 200 |
| EMPLOYED_BY | Person → Company | 雇佣关系【新增】 | 80 |
| ADMIN_PENALTY_OF | AdminPenalty → Company | 行政处罚关系【新增】 | 12 |
| BUSINESS_ABNORMAL_OF | BusinessAbnormal → Company | 经营异常关系【新增】 | 26 |
...
| DaaS 外部数据 | AdminPenalty【新增】 | 12 |
| DaaS 外部数据 | BusinessAbnormal【新增】 | 27 |
...
```
USER_001 -[:EMPLOYED_BY]-> ORG_001 (董事长, 2015-01-08)
USER_021 -[:EMPLOYED_BY]-> ORG_001 (副总经理, 2017-09-23)
USER_041 -[:EMPLOYED_BY]-> ORG_001 (董事长, 2019-05-13)
USER_061 -[:EMPLOYED_BY]-> ORG_001 (副总经理, 2021-01-03)
```
...
**核心特性**：
- 行政处罚分析：处罚次数、金额、类型、时间
- 经营异常分析：异常次数、列入原因、是否已移出
- 风险传导：通过控股、交易关系传导到关联方
...
- HAS_PARTY: 200条（自动生成）
- EMPLOYED_BY: 80条【新增】
- ADMIN_PENALTY_OF: 12条【新增】
- BUSINESS_ABNORMAL_OF: 26条【新增】
- **总计**: 1089条边
...
### 8.1 数据生成流程
```
enhanced_mock_data/ (CSV源数据)
    ↓
generate_enhanced_graph_data.py (数据转换)
    ↓
enhanced_graph_data/ (图格式CSV)
    ↓
nebula_import.py (数据导入)
    ↓
Nebula Graph (图数据库)
```
...
Nebula Graph
    ↓
分析脚本 (fraud_rank.py, circular_trade.py等)
    ↓
CSV报告 (reports/)
    ↓
可视化 (visualization.py, web_demo.py)
```

### 8.3 关键技术
- **图数据库**: Nebula Graph
- **图算法**: PageRank (FraudRank), DFS (循环交易检测)
- **数据分析**: Pandas, NumPy
- **可视化**: Plotly, NetworkX, D3.js
- **Web框架**: Flask
...
### v3.0 (当前版本)
- 新增 AdminPenalty（行政处罚）节点类型
- 新增 BusinessAbnormal（经营异常）节点类型
- 新增 EMPLOYED_BY（雇佣关系）边类型
- 新增 ADMIN_PENALTY_OF（行政处罚关系）边类型
- 新增 BUSINESS_ABNORMAL_OF（经营异常关系）边类型
- 新增外部风险事件分析场景
- 新增人员任职网络分析场景
- 支持 DaaS 外部风险数据导入
...
### v2.0
- 新增 Transaction 节点类型
- 新增 IS_SUPPLIER、IS_CUSTOMER、PAYS、RECEIVES、HAS_PARTY 边类型
- 移除 Company.company_type 属性
- 新增履约关联风险检测场景

### v1.0
- 初始版本
- 基础节点：Person、Company、Contract、LegalEvent
- 基础边：LEGAL_PERSON、CONTROLS、PARTY_*、TRADES_WITH、INVOLVED_IN、RELATED_TO
