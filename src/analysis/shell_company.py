...
def calculate_shell_company_score(features):
    ...
    if features["contract_count"] > 0:
        avg_contract_amount = (
            features["total_inflow"] + features["total_outflow"]
        ) / features["contract_count"]
...
def main():
    ...
    try:
        ...
        print(
            f"  高嫌疑空壳公司数量: {len(high_risk)} ({len(high_risk) / len(features_df) * 100:.1f}%)"
        )
        ...
        if len(high_risk) > 0:
            ...
            if shell_networks:
                ...
                for i, network in enumerate(shell_networks[:3], 1):
                    ...
                    print(
                        f"    公司列表: {', '.join(str(c) for c in network['companies'][:5])}..."
                    )
