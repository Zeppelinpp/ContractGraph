...
def risk_gradient_view(
    ...
    try:
        ...
        if size_by == "pagerank":
            ...
            node_sizes = np.array(
                [pagerank_scores.get(node_id, 0.0) for node_id in node_ids_list]
            )
        else:
            node_sizes = np.array(
                [graph["out_degree"].get(node_id, 0) for node_id in node_ids_list]
            )
        ...
        # 获取 FraudScore（用于颜色）
        fraud_values = np.array(
            [fraud_scores.get(node_id, 0.0) for node_id in node_ids_list]
        )
        ...
        if len(high_risk_indices) > 0:
            ...
            print(
                f"  高风险节点中心位置: ({high_risk_center[0]:.2f}, {high_risk_center[1]:.2f})"
            )
            print(
                f"  高风险节点分布标准差: ({high_risk_std[0]:.2f}, {high_risk_std[1]:.2f})"
            )
        ...
        if len(low_risk_indices) > 0 and len(high_risk_indices) > 0:
            ...
            if distances:
                ...
                if min_dist < 2.0:
                    print(
                        f"  ⚠ 警告：发现低风险节点与高风险节点距离很近（最小距离: {min_dist:.2f}）"
                    )
...
def implicit_link_view(
    ...
    try:
        ...
        print(
            f"  发现隐式关联数: {len(implicit_links)} (相似度 >= {similarity_threshold})"
        )
        ...
        for i, j, similarity in implicit_links:
            ...
            ax.plot(x_coords, y_coords, "--", color="red", alpha=0.3, linewidth=0.5)
        ...
        if implicit_links:
            ...
            for node_type, count in sorted(
                type_groups.items(), key=lambda x: x[1], reverse=True
            ):
...
def interactive_risk_exploration(
    ...
    try:
        ...
        implicit_links = implicit_links[:max_links]
        print(
            f"  Found {len(implicit_links)} implicit links (similarity >= {similarity_threshold})"
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "out_degree": [
                    graph["out_degree"].get(nid, 0) for nid in node_ids_list
                ],
            }
        )
        ...
        if implicit_links:
            ...
            for i, j, sim in implicit_links:
                link_x.extend(
                    [reduced_embeddings[i, 0], reduced_embeddings[j, 0], None]
                )
                link_y.extend(
                    [reduced_embeddings[i, 1], reduced_embeddings[j, 1], None]
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        ...
                        colorbar=dict(
                            title=dict(text="Fraud Score", font=dict(size=12))
                        )
                        if show_colorbar
                        else None,
