...
TAG_TYPES = ["Person", "Company", "Contract", "LegalEvent", "Transaction", "AdminPenalty", "BusinessAbnormal"]
    ...
    for attempt in range(retry):
        ...
        if error_msg:
            try:
                ...
                if "more than one request" in error_msg_lower or "concurrent" in error_msg_lower:
                    if attempt < retry - 1:
...
def import_edges(session):
    ...
    import_edges_from_file(session, "edges_admin_penalty_company.csv", "ADMIN_PENALTY_OF")
    import_edges_from_file(session, "edges_business_abnormal_company.csv", "BUSINESS_ABNORMAL_OF")
